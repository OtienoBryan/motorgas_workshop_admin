...
function quotationFinancials(jc: JobCard) {
...
const Quotations: React.FC = () => {
  ...
  const fetchJobCards = async () => {
    try {
      ...
      // Only job cards still in the quotation stage belong here — converted ones are invoices
      const quotations = (Array.isArray(data) ? data : []).filter(jc => ESTIMATE_STAGE_STATUSES.includes(jc.status))
      setJobCards(quotations)
    } catch {
      setJobCards([])
    } finally {
      setLoading(false)
  ...
  const totals = jobCards.reduce((acc, jc) => acc + quotationFinancials(jc).total, 0)
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="text-white px-5 py-3" style={{ backgroundColor: '#0b0f24' }}>
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-3 flex-1 min-w-0">
            <h1 className="text-sm font-bold whitespace-nowrap">Quotations</h1>
      ...
      <div className="px-5 pt-4">
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 flex items-center gap-3 max-w-xs">
          <div className="w-9 h-9 rounded-xl bg-blue-50 flex items-center justify-center shrink-0">
            <Wallet className="h-4 w-4 text-blue-600" />
          </div>
          <div className="min-w-0">
            <p className="text-[10px] text-gray-400 uppercase tracking-wide">Total Quoted</p>
            <p className="text-sm font-bold text-gray-900 truncate">{money(totals)}</p>
          </div>
        </div>
      </div>
        ...
        {filtered.length === 0 ? (
          <div className="bg-white rounded-2xl border border-gray-100 flex flex-col items-center justify-center py-16 text-center">
            ...
            <p className="text-xs text-gray-400 mt-1">
              {searchTerm ? 'Try adjusting your search' : 'Quotations are created from the Job Cards page'}
            </p>
          </div>
        ) : (
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-gray-100 bg-gray-50">
                  <th className="px-4 py-1.5 text-left text-[10px] font-semibold text-gray-400 uppercase tracking-wide">Quotation #</th>
              ...
              <tbody className="divide-y divide-gray-50">
                {paginated.map(jc => {
                  const fin = quotationFinancials(jc)
                  return (
                    <tr key={jc.id}
                      ...
                      <td className="px-4 py-1.5">
                        <span className={`inline-block px-2 py-0.5 rounded-full text-[10px] font-medium whitespace-nowrap ${JOB_STATUS_STYLES[jc.status] || 'bg-gray-100 text-gray-500'}`}>
                          {JOB_STATUS_LABELS[jc.status] || jc.status}
                        </span>
                      </td>
                      <td className="px-4 py-1.5 text-right">
                        <button
                          onClick={(e) => { e.stopPropagation(); navigate(`/job-cards/${jc.id}`) }}
