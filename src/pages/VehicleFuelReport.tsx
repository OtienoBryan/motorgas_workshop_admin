...
import { adminApiService, VehicleFuelReport as FuelReport, FuelReportVehicle, Station } from '../services/api'
import { ArrowLeft, Search, Download, RefreshCw, FileText, Loader2, AlertTriangle } from 'lucide-react'
...
const VehicleFuelReport: React.FC = () => {
  ...
  const [lowFuelThreshold, setLowFuelThreshold] = useState(1)
  ...
  const fetchReport = async () => {
    try {
      setLoading(true)
      const data = await adminApiService.getVehicleFuelReport(startDate, endDate, selectedStationId || undefined)
      setReport(data)
    } catch (error) {
      console.error('❌ [VehicleFuelReport] Error fetching report:', error)
      setReport(null)
    } finally {
      setLoading(false)
    }
  ...
  const fetchStations = async () => {
    try {
      ...
      console.error('❌ [VehicleFuelReport] Error fetching stations:', error)
      setStations([])
    }
  }
  ...
  // Month groups with the ISO weeks each month contains (Performance section)
  ...
  const weekFills = (vehicle: FuelReportVehicle, monthKey: string, week: number): number =>
    days
      .filter(d => d.monthKey === monthKey && d.week === week)
      .reduce((sum, d) => sum + (vehicle.dailyFills[d.dateStr] || 0), 0)

  const monthFills = (vehicle: FuelReportVehicle, monthKey: string): number =>
    days
  ...
  const formatAmount = (v: number): string =>
    v === 0 ? '0' : v.toLocaleString(undefined, { maximumFractionDigits: 0 })
  ...
  const isLowFuel = (v: FuelReportVehicle): boolean => v.totalFills < lowFuelThreshold
  const lowFuelCount = filteredVehicles.filter(isLowFuel).length
  ...
  const exportToCSV = () => {
    ...
    const headers = [
      ...
      'Total Fills', 'Total Quantity (L)', 'Total Amount', 'Low Fueling',
    ]
    ...
    const rows = filteredVehicles.map((v, idx) => [
      ...
      escapeCSV(v.driver),
      escapeCSV(v.tel),
      ...months.flatMap(m => [...m.weeks.map(w => formatNum(weekFills(v, m.key, w))), formatNum(monthFills(v, m.key))]),
      ...inputDays.map(d => formatNum(v.dailyFills[d.dateStr] || 0)),
      v.totalFills,
      formatNum(v.totalQuantity),
    ...
    link.setAttribute('download', `vehicle-fuel-report-${startDate}-to-${endDate}.csv`)
  ...
  const exportToPdf = async () => {
    ...
    try {
      ...
      await exportElementToPdf(wrapper, `vehicle-fuel-report-${startDate}-to-${endDate}.pdf`, {
        orientation: 'l',
        header: {
  ...
  const columnTotal = (getter: (v: FuelReportVehicle) => number): number =>
    ...
    <div className="min-h-screen bg-gray-50 p-2">
      <div className="max-w-full mx-auto">
        ...
        <div className="mb-2 flex items-center gap-2">
          ...
          <h1 className="text-xs font-bold text-gray-900">Vehicle Fuel Report</h1>
          {lowFuelCount > 0 && (
            <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-medium bg-[#e9e4f5] text-[#5b4ba8]">
              <AlertTriangle className="h-2.5 w-2.5" />
              {lowFuelCount} low fueling
            </span>
          )}
        </div>

        {/* Filters */}
        <div className="mb-2 grid grid-cols-3 md:grid-cols-6 gap-2">
          <div>
          ...
          <div>
            <label className="block text-[10px] font-medium text-gray-700 mb-0.5">Low Fuel Threshold (fills)</label>
            <input
              type="number"
              min={0}
              value={lowFuelThreshold}
              onChange={(e) => setLowFuelThreshold(Math.max(0, parseInt(e.target.value) || 0))}
              className="w-full px-1.5 py-0.5 text-[10px] border border-gray-300 rounded focus:ring-1 focus:ring-blue-500 focus:border-blue-500"
              title="Vehicles with fewer fills than this are highlighted"
            />
          </div>
          <div>
        ...
        <div ref={tableWrapperRef} className="bg-white rounded border overflow-hidden">
          {loading ? (
            ...
            <div className="text-center py-8 text-[10px] text-gray-500">No vehicles found for this period</div>
          ) : (
              ...
              <table className="border-collapse text-[10px]" style={{ minWidth: '100%' }}>
                <thead>
                  ...
                  <tr>
                    ...
                    <th colSpan={inputDays.length + 3} className="border border-gray-300 bg-[#f4c7c3] text-gray-900 font-bold py-1 px-2 text-center">
                      INPUT
                    </th>
                  </tr>
                  {/* Sub header */}
                  <tr>
                    ...
                    <th rowSpan={3} className="border border-gray-300 bg-white font-semibold px-1 align-bottom text-center">
                      <div className={rotatedHeaderClass}>Total Fills</div>
                    </th>
                    <th rowSpan={3} className="border border-gray-300 bg-white font-semibold px-1 align-bottom text-center">
                      <div className={rotatedHeaderClass}>Total Qty (L)</div>
                    </th>
                    <th rowSpan={3} className="border border-gray-300 bg-white font-semibold px-1 align-bottom text-center">
                      <div className={rotatedHeaderClass}>Amount (KES)</div>
                    </th>
                  </tr>
                  {/* Month names */}
                  <tr>
                ...
                <tbody>
                  {filteredVehicles.map((vehicle, idx) => {
                    const lowFuel = isLowFuel(vehicle)
                    const stickyBg = lowFuel ? 'bg-[#e9e4f5]' : 'bg-white'
                    return (
                      <tr key={vehicle.key} className={lowFuel ? 'bg-[#f3f0fa]' : 'hover:bg-blue-50'}>
                        <td className={`border border-gray-300 ${stickyBg} text-center px-1 py-0.5`} style={stickyStyle(0)}>
                          {idx + 1}
                        </td>
                        <td className={`border border-gray-300 ${stickyBg} font-medium px-1 py-0.5 whitespace-nowrap`} style={stickyStyle(1)}>
                          {vehicle.regNo}
                        </td>
                        <td className={`border border-gray-300 ${stickyBg} px-1 py-0.5 whitespace-nowrap overflow-hidden text-ellipsis text-center`} style={stickyStyle(2)}>
                          {vehicle.org || '-'}
                        </td>
                        <td className={`border border-gray-300 ${stickyBg} px-1 py-0.5 whitespace-nowrap overflow-hidden text-ellipsis text-center`} style={stickyStyle(3)}>
                          {vehicle.driver || '-'}
                        </td>
                        <td className={`border border-gray-300 ${stickyBg} px-1 py-0.5 whitespace-nowrap text-center`} style={stickyStyle(4)}>
                          {vehicle.tel || '-'}
                        </td>
                        {months.map((m) => (
                          <React.Fragment key={m.key}>
                            {m.weeks.map((w) => (
                              <td key={w} className="border border-gray-300 text-right px-1 py-0.5">
                                {formatNum(weekFills(vehicle, m.key, w))}
                              </td>
                            ))}
                            <td className="border border-gray-300 text-right font-semibold px-1 py-0.5" style={monthDivider}>
                              {formatNum(monthFills(vehicle, m.key))}
                            </td>
                          </React.Fragment>
                        ))}
                        {inputDays.map((d) => (
                          <td key={d.dateStr} className="border border-gray-300 text-right px-1 py-0.5">
                            {formatNum(vehicle.dailyFills[d.dateStr] || 0)}
                          </td>
                        ))}
                        <td className={`border border-gray-300 text-right font-bold px-1 py-0.5 ${lowFuel ? 'text-[#5b4ba8]' : ''}`}>
                          {vehicle.totalFills}
                        </td>
                        <td className="border border-gray-300 text-right font-semibold px-1 py-0.5">
                          {formatNum(vehicle.totalQuantity)}
                        </td>
                        <td className="border border-gray-300 text-right font-semibold px-1 py-0.5 whitespace-nowrap">
                          {formatAmount(vehicle.totalAmount)}
                        </td>
                      </tr>
                    )
                  })}
                  ...
                  <tr className="bg-gray-100 font-bold">
                    ...
                    {months.map((m) => (
                      <React.Fragment key={m.key}>
                        {m.weeks.map((w) => (
                          <td key={w} className="border border-gray-300 text-right px-1 py-0.5">
                            {formatNum(columnTotal(v => weekFills(v, m.key, w)))}
                          </td>
                        ))}
                        <td className="border border-gray-300 text-right px-1 py-0.5" style={monthDivider}>
                          {formatNum(columnTotal(v => monthFills(v, m.key)))}
                        </td>
                      </React.Fragment>
                    ))}
                    {inputDays.map((d) => (
                      <td key={d.dateStr} className="border border-gray-300 text-right px-1 py-0.5">
                        {formatNum(columnTotal(v => v.dailyFills[d.dateStr] || 0))}
                      </td>
                    ))}
                    <td className="border border-gray-300 text-right px-1 py-0.5">
                      {formatNum(columnTotal(v => v.totalFills))}
                    </td>
                    <td className="border border-gray-300 text-right px-1 py-0.5">
                      {formatNum(columnTotal(v => v.totalQuantity))}
                    </td>
                    <td className="border border-gray-300 text-right px-1 py-0.5 whitespace-nowrap">
                      {formatAmount(columnTotal(v => v.totalAmount))}
                    </td>
                  </tr>
                </tfoot>
              </table>
        ...
        <div className="mt-2 flex items-center gap-2 text-[10px] text-gray-500">
          <span className="inline-block w-3 h-3 rounded-sm bg-[#e9e4f5] border border-[#cfc7ea]" />
          Low fueling — fewer than {lowFuelThreshold} fill{lowFuelThreshold !== 1 ? 's' : ''} in the selected period
        </div>
      </div>
    </div>
  )
